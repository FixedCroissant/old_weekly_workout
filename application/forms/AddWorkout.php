...
class Form_AddWorkout extends App_Form
{
	public function __construct($options = null)
	{
		parent::__construct($options);

		$this->setAttrib('id','add_workout')
			->setName('AddWorkout');
		$date = new Zend_Date();

		// element: workout_month
		$date_copy = clone $date; //prevent date object errors
		$current_month = $date->get(Zend_Date::MONTH_NAME_SHORT);
		$pm = $date_copy->subMonth('1', Zend_Date::MONTH);
		$previous_month = $pm->get(Zend_Date::MONTH_NAME_SHORT);
		...
		$wklength->setLabel('workout_label_wklength')
				...
				->addValidator('NotEmpty')
				->addValidator('Int')
				->addValidator('Between', true, array(0,999));
		...
		$elements = array($month, $day, $year, $prehr, $peakhr, $posthr,
		$pushups, $pushuptype, $crunches, $wktype, $wklength, $add);
